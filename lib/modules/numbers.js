...
const checkIsInteger = (k, v, obj) => {
	if (!_.isInteger(obj[k])) {
		let err = new Error(`Value passed -> ${obj[k]} is not an integer of key: ${k}`);
		return { err, message: 'Value not integer error', result: 'invalid' };
	}

	return { err: null, message: 'Validation successful', result: 'valid' };
};

module.exports = { checkMin, checkMax, checkRange, checkIsInteger };
